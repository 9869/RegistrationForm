...
public class LoginDao {
	public String authenticateUser(LoginBean loginBean) {
		...
		String passwordDB = "";
		try {
			con = LoginDBC.createConnection(); // establishing connection
			statement = con.createStatement(); // Statement is used to write queries. Read more about it.
			resultSet = statement.executeQuery("select Email,Password from btm.users"); // Here table name is
																								// users and
																								// userName,password are
																								// columns. fetching all
																								// the records and
																								// storing in a
																								// resultSet.
